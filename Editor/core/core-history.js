// core-history.js – Undo/Redo + Compound-Actions
// Usage:
//   editor.use(EditorHistory);
//   editor.pushAction({ type:"setTiles", changes:[{level,layer,col,row,prev,next}] });
//   editor.beginCompound("Brush"); /* ... viele setTile()/pushAction() ... */ editor.endCompound();
//   editor.undo(); editor.redo(); const info = editor.getUndoInfo(); // {undo,redo,limit,inCompound}
// API:
//   pushAction(act), beginCompound(label), endCompound([cancel]),
//   undo(), redo(), clearHistory(), setUndoLimit(n), getUndoInfo()
// Notes:
//   Replay erfolgt über setTileRaw(...). Jede neue Aktion leert den Redo-Stack. :contentReference[oaicite:43]{index=43}
