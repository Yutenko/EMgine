// core-camera.js – Kamera (Pan/Zoom) + Koordinaten
// Usage:
//   editor.use(EditorCamera);
//   editor.applyCamera(ctx); // vor dem Zeichnen
//   const cam = editor.getCamera(); editor.panTo(x,y); editor.zoomBy(1.2, sx, sy);
//   const wpt = editor.screenToWorld(sx,sy), t = editor.clientToTile(evt);
// API:
//   getCamera(), applyCamera(ctx)
//   screenToWorld(), worldToScreen(), worldToTile(), screenToTile(), clientToTile()
//   panTo(x,y), zoomBy(factor[, sx,sy])
// Notes:
//   Kamera wird bei "canvas:resize"/"world:resize" geclamped; Zoom zentriert optional um (sx,sy). 
