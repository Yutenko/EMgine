(function () {
  ...
  // Tools: Paint, Pan, Zoom In, Zoom Out, Undo, Redo
  // Include `editor-toolbar.css` for styling; if it's not present, this script injects fallback styles.
  // Communicates via:
  //  - window.editor.undo()/redo()/zoomBy()
  //  - CustomEvent('em:setTool', {detail:{tool:'paint'|'pan'}})
  //  - window.emSetTool(tool) to sync from other UIs
  ...
  function injectFallbackStyles() {
    ...
    var css = [
      ...
      "." +
        BTN_CLS +
        "{min-width:36px;min-height:36px;display:inline-flex;align-items:center;justify-content:center;padding:6px 10px;border-radius:10px;border:1px solid rgba(255,255,255,0.12);background:#1e232b;color:#e0e6ea;cursor:pointer;user-select:none;font:600 13px/1 system-ui,-apple-system,Segoe UI,Roboto,Arial;transition:transform .05s ease, background .15s ease, border-color .15s ease;box-shadow:inset 0 0 0 1px rgba(255,255,255,0.03);}",
      "." + BTN_CLS + ":hover{background:#242a33;}",
      "." + BTN_CLS + ":active{transform:translateY(1px);}",
      "." +
        BTN_ACTIVE_CLS +
        "{border-color:#64b5f6;box-shadow:inset 0 0 0 1px #64b5f6;background:#263445;color:#e7f3ff;}",
      "." +
        DISABLED_CLS +
  ...
  function makeButton(key, title, onClick) {
    ...
    b.textContent = icons[key] || title[0] || "?";
  ...
  function selectTool(tool) {
    ...
    if (root) {
      var btns = root.querySelectorAll(
        "." + BTN_CLS + "[data-key='paint'], ." + BTN_CLS + "[data-key='pan']"
      );
  ...
  function build() {
    ...
    // enable/disable undo/redo polling
    function pollUndoRedo() {
      var canU = canCall(window.editor, "canUndo")
        ? !!window.editor.canUndo()
        : true;
      var canR = canCall(window.editor, "canRedo")
        ? !!window.editor.canRedo()
        : true;
