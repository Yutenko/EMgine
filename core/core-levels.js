...
(function(){
  function install(editor){
    ...
    var LAYER_TINT = { floor:"#4FC3F7", wall:"#FF8A65", decor:"#BA68C8", entities:"#FFF176" };
    ...
    function pickLayerForLevel(levelIndex){
      ...
      if (lastLayerGlobal && LAYERS.indexOf(lastLayerGlobal)!==-1) return lastLayerGlobal;
      return "floor";
    }
    ...
    function levelsInit(){
      ...
      for (var l=0;l<LEVELS.count;l++){
        var lvl = {};
        for (var i=0;i<LAYERS.length;i++) lvl[LAYERS[i]] = new Uint16Array(LEVELS.size);
        LEVELS.data.push(lvl);
      }
      lastLayerByLevel = new Array(LEVELS.count);
      ensureLayerMemorySize();
      if (!LEVELS.layer || LAYERS.indexOf(LEVELS.layer)===-1){ LEVELS.layer="floor"; lastLayerGlobal="floor"; }
    ...
    editor.on("world:resize", function(){
      ...
      var cols=w.cols|0, rows=w.rows|0;
      ...
      for (var l=0;l<LEVELS.data.length;l++){
        ...
        for (var i=0;i<LAYERS.length;i++){
          ...
          for (var r=0;r<copyRows;r++){
            var o = r*LEVELS.cols, n=r*cols;
            for (var c=0;c<copyCols;c++) next[n+c] = old[o+c];
          }
          lvl[name] = next;
        }
      }
      LEVELS.cols=cols; LEVELS.rows=rows; LEVELS.size=newSize;
      if (editor.markAllLayerChunksDirty) editor.markAllLayerChunksDirty();
      if (editor.clearHistory) editor.clearHistory();
      editor.emit("levels:resize", {cols:cols, rows:rows});
    });

    function getLevelsCount(){ return LEVELS.count|0; }
    function getCurrentLevel(){ return LEVELS.current|0; }
    ...
    function getCurrentLayer(){ return LEVELS.layer; }
    ...
    function isLayerVisible(name){ return !!LEVELS.show[name]; }
    function setLayerVisible(name, on){ LEVELS.show[name] = !!on; editor.requestRender(); }

    function inWorldBounds(col,row){
      return !(col<0 || row<0 || col>=(LEVELS.cols|0) || row>=(LEVELS.rows|0));
    }

    function levelTileIndex(col,row){ return row*(LEVELS.cols|0) + col; }
    function getActiveBuffer(){ return LEVELS.data[LEVELS.current|0]; }

    function colorForIdLayer(layer, id){
      var base = LAYER_TINT[layer] || "#9E9E9E";
      function hexToRgb(h){ var s = h.charAt(0)==="#" ? h.substring(1):h; var n=parseInt(s,16); return {r:(n>>16)&255,g:(n>>8)&255,b:n&255}; }
      function rgbToHex(r,g,b){ return "#"+(((1<<24)+((r&255)<<16)+((g&255)<<8)+(b&255))>>>0).toString(16).slice(1).toUpperCase(); }
      function varyById(tintHex, idv){ var rgb=hexToRgb(tintHex); var f=0.85 + (((idv*1103515245 + 12345)>>>0)%300)/1000; var r=Math.max(0,Math.min(255,(rgb.r*f)|0)); var g=Math.max(0,Math.min(255,(rgb.g*f)|0)); var b=Math.max(0,Math.min(255,(rgb.b*f)|0)); return rgbToHex(r,g,b); }
    ...
    function setTileRaw(level, layer, col, row, id){
      ...
      if ((level|0) === (LEVELS.current|0)) {
        if (editor.markLayerChunkDirtyAt) editor.markLayerChunkDirtyAt(layer, col, row);
      } else {
        if (editor.markLevelLayerChunkDirty) editor.markLevelLayerChunkDirty(level|0, layer, col, row);
      }
    }

    function setTile(col,row,id){
      col=col|0; row=row|0; id=id|0;
      ...
      if (editor.pushAction){
        editor.pushAction({type:"setTiles", changes:[{level:LEVELS.current|0, layer:LEVELS.layer, col:col, row:row, prev:prev|0, next:id|0}]});
      }
      setTileRaw(LEVELS.current|0, LEVELS.layer, col, row, id);
      editor.emit("tiles:changed", {col:col,row:row,layer:LEVELS.layer,level:LEVELS.current|0});
      editor.requestRender();
      return true;
    }

    function setHoverLayer(name){ LEVELS.hoverLayer = name || null; editor.requestRender(); }
